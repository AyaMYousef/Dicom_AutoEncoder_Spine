...
(function() {
  var SOPClass, TransferSyntax, UID, find, for_uid, vrs, _UID_DICT, _make_uid,
  ...
  TransferSyntax = (function(_super) {
    __extends(TransferSyntax, _super);

    function TransferSyntax() {
      return TransferSyntax.__super__.constructor.apply(this, arguments);
    }

    TransferSyntax.prototype.endianess = function() {
      if (/^ExplicitVRBigEndian/.test(this.name)) {
        return vrs.BIG_ENDIAN;
      } else {
        return vrs.LITTLE_ENDIAN;
      }
    };

    TransferSyntax.prototype.is_explicit = function() {
      return !/^Implicit/.test(this.name);
    };

    TransferSyntax.prototype.make_context = function() {
      var obj;
      obj = {
        endianess: this.endianess(),
        explicit: this.is_explicit()
      };
      return obj;
    };

    TransferSyntax.prototype.value_length_bytes = function(vr) {
      if (this.is_explicit()) {
        return vr.explicit_value_length_bytes;
      } else {
        return vr.implicit_value_length_bytes;
      }
    };
  ...
  SOPClass = (function(_super) {
    __extends(SOPClass, _super);

    function SOPClass() {
      return SOPClass.__super__.constructor.apply(this, arguments);
    }
  ...
  _make_uid = function(uid, name, typ) {
    var constr;
    constr = (function() {
      switch (typ) {
        case 'TransferSyntax':
    ...
    return new constr(uid, name, typ);
  };

  for_uid = function(uid) {
    return _UID_DICT[uid] || exports[uid];
  };

  find = function(regex) {
    var uid, uidObj;
    for (uid in _UID_DICT) {
      uidObj = _UID_DICT[uid];
      if (regex.test(uid) || regex.test(uidObj.name)) {
        console.log(uidObj);
      }
    }
    return void 0;
